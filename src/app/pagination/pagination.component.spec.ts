...
describe('PaginationComponent', () => {
  ...
  it('should render pagination controls when totalPages is provided', () => {
    ...
    const pageNumberButtonsContainer = paginationContainer.querySelector(
      '.flex.justify-center.space-x-2'
    );
    expect(pageNumberButtonsContainer).toBeTruthy();
    expect(
      pageNumberButtonsContainer.querySelectorAll(
        'a.bg-blue-100.text-blue-500.px-3.py-2.rounded-md'
      ).length
    ).toBe(10);

    const nextButton = paginationContainer.querySelector(
      '.bg-gray-700.text-white.px-3.py-2.rounded-md:last-child'
    );
